#!/usr/bin/env python3
"""
EtherCAT driver for the LEFT Inspire RH56F1 dexterous hand (single hand).

Reads hand state in a loop via cyclic PDO exchange.
Requires root/sudo for raw socket access.

Usage:
    sudo python ethercat_driver_l.py
    sudo python ethercat_driver_l.py --ifname enp3s0 --slave 1
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Inspire RH56F1 EtherCAT Driver - LEFT hand')
    parser.add_argument('--ifname', type=str, default='eth0',
                        help='Network interface (default: eth0)')
    parser.add_argument('--slave', type=int, default=1,
                        help='EtherCAT slave index for left hand (default: 1)')
    parser.add_argument('--cycle', type=float, default=2.0,
                        help='PDO cycle time in ms (default: 2.0)')
    args = parser.parse_args()

    print(f"[LEFT] EtherCAT on {args.ifname}, slave={args.slave}, cycle={args.cycle}ms")
